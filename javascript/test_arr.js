...
$(function () {
    ...
    for (var i = 0, len = cateJSON.length; i < len; i++) {
        if (cateJSON[i].parentID == cateJSON[a].parentID) {
            arrCate.push(cateJSON[i]);
        }
    }
    console.log(arrCate);
    a = usual_search(arrCate, x)
    c = Number(a) + Number(1);
    var jumpUrl = "";
    jumpIndex = "index.html?pageSize=" + (Number(pageSize) + Number(c_next)) + "&id=" + arrCate[a].parentID + "&c_audio=" + c_audio;
    if (arrCate[c] !== undefined) {
        if (arrCate[a].parentID == arrCate[c].parentID) {
            if (c_mode == 1) {
                if (pageSize > comments.length) {
                    //alert("1");
                    jumpUrl = "test.html?id=" + arrCate[c].ID + "&pageSize=1&c_audio=" + c_audio + "&c_mode=" + c_mode;
                } else {
                    //alert("2");
                    pageSize = Number(pageSize) + Number(c_nextSize);
                    jumpUrl = "test.html?id=" + arrCate[a].ID + "&pageSize=" + pageSize + "&c_audio=" + c_audio + "&c_mode=" + c_mode; //jump to the beginning and pagesize+1
                }
            }
            if (c_mode == 0) {
                jumpUrl = "test.html?id=" + arrCate[c].ID + "&pageSize=" + pageSize + "&c_audio=" + c_audio + "&c_mode=" + c_mode; //jump to next question category
            }
            if (c_mode == 2) {
                jumpUrl = "test.html?id=" + arrCate[c].ID + "&pageSize=" + pageSize + "&c_audio=" + c_audio + "&c_mode=" + c_mode; //jump to next question catego	
            }
        } else {
    ...
    title = cateJSON[usual_search(cateJSON, pid)].title;
    $("title").text(title); //change title
    $(".spPageSize").html(pageSize);
    $(".spTitle").html(title); //change title
    ...
    function act() {
        ...
        if (index < comments.length) {					//如果指针在范围内；
            ...
            //=================================================================================	 BEGIN
            $(".f_jsEdit").attr("href", "http://localhost/net_test/net_test/ListsSpry/Edit/" + comments[index]['ID']);
